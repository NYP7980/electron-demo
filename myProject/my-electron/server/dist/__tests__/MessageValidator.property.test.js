...
describe('MessageValidator — Property 14: Malformed messages are rejected', () => {
    ...
    test('non-object values are always rejected', () => {
        fc.assert(fc.property(fc.oneof(fc.integer(), fc.float(), fc.string(), fc.boolean(), fc.constant(null), fc.constant(undefined), fc.array(fc.anything())), (value) => {
            const result = (0, MessageValidator_1.validateMessage)(value);
    ...
    test('objects with missing or wrong-typed "type" field are rejected', () => {
        fc.assert(fc.property(fc.record({
            ...
            const result = (0, MessageValidator_1.validateMessage)(msg);
            expect(result.ok).toBe(false);
        }), { numRuns: 200 });
    });
    test('objects with unknown type strings are rejected', () => {
        ...
        fc.assert(fc.property(fc.string().filter(s => !knownTypes.has(s)), (unknownType) => {
            const result = (0, MessageValidator_1.validateMessage)({ type: unknownType });
            expect(result.ok).toBe(false);
        }), { numRuns: 200 });
    });
    test('join_room with missing or wrong-typed roomId is rejected', () => {
        fc.assert(fc.property(fc.oneof(fc.integer(), fc.boolean(), fc.constant(null), fc.constant(undefined)), (badRoomId) => {
            const result = (0, MessageValidator_1.validateMessage)({ type: 'join_room', roomId: badRoomId, nickname: null });
            expect(result.ok).toBe(false);
        }), { numRuns: 200 });
    });
    test('join_room with wrong-typed nickname is rejected', () => {
        fc.assert(fc.property(fc.oneof(fc.integer(), fc.boolean(), fc.array(fc.string())), (badNickname) => {
            const result = (0, MessageValidator_1.validateMessage)({ type: 'join_room', roomId: 'room1', nickname: badNickname });
            expect(result.ok).toBe(false);
        }), { numRuns: 200 });
    });
    test('make_move with missing seq or non-object move is rejected', () => {
        fc.assert(fc.property(fc.record({
            ...
            const result = (0, MessageValidator_1.validateMessage)({ type: 'make_move', move, seq });
            expect(result.ok).toBe(false);
        }), { numRuns: 200 });
    });
    test('select_game with invalid gameType is rejected', () => {
        const validTypes = ['gomoku', 'army-chess', 'jungle-chess'];
        fc.assert(fc.property(fc.string().filter(s => !validTypes.includes(s)), (badType) => {
            const result = (0, MessageValidator_1.validateMessage)({ type: 'select_game', gameType: badType });
            expect(result.ok).toBe(false);
        }), { numRuns: 200 });
    });
